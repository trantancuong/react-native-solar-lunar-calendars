...
import {formatNumbers, weekDayNames2} from '../../dateutils';
...
const CalendarHeader = forwardRef((props: CalendarHeaderProps, ref) => {
    ...
    const renderWeekDays = useMemo(() => {
        ...
        const weekDaysNames = numberOfDaysCondition ? weekDayNames2(weekdaysShort, dayOfTheWeek) : weekDayNames2(weekdaysShort, firstDay);
        ...
        return dayNames.map((day: string, index: number) => {
            ...
            return (
                <Text allowFontScaling={false} key={index} style={[dayStyle, {textTransform: "capitalize"}]}
                      numberOfLines={1} accessibilityLabel={''}>
                    {day}
                </Text>
            );
        });
