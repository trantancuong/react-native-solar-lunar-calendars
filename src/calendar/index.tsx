...
const Calendar = (props: CalendarProps & ContextProp) => {
    ...
    const localeConfig = (JSON.parse(JSON.stringify(props.locale.config)));
    const weekdaysShort = (localeConfig['__weekdaysShort']);
    // XDate.defaultLocale = props.locale?.name;
    // XDate.locales[XDate.defaultLocale] = props.locale?.config;
    ...
    const renderDay = (day: XDate, id: number) => {
        ...
        return (
            <View style={[style.current.dayContainer, {
                height: 21 //Custom
            }]} key={id}>
                <Day
                    {...dayProps}
    ...
    const renderFooter = () => {
        const footerProps = extractHeaderProps(props);
        const ref = customFooter ? undefined : header;
        const CustomFooter = customFooter;
        const FooterComponent = customFooter ? CustomFooter : CalendarFooter;
        ...
        return (
            <FooterComponent
                ...
                testID={`${testID}.header`}
                style={[footerStyle]}
                renderCurrent={props.renderCurrent}
                ref={ref}
                month={currentMonth}
                addMonth={addMonth}
                displayLoadingIndicator={shouldDisplayIndicator}
...
Calendar.propTypes = {
    ...
    footerStyle: PropTypes.oneOfType([PropTypes.object, PropTypes.number, PropTypes.array]),
